...
class OntologyEnricher:
    """Semantic mapper for ontology class alignment."""
    ...
    def _is_valid_concept(self, text):
        """Check if text represents a valid concept (noun-based, no verbs)."""
        doc = nlp(text)
    ...
    def load_ontology(self, file_path):
        """Load ontology and extract class embeddings."""
        g = Graph()
        try:
            g.parse(file_path)
        except Exception:
    ...
    def run(self, input_xlsx, ontology_path, output_xlsx):
        """Execute semantic mapping pipeline."""
